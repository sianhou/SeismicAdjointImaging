...
{
     int i,j;
     float *y = sjmflloc1d(n);
	 float *yh= sjmflloc1d(n);
	 float *ye= sjmflloc1d(n);
	 float *u = sjmflloc1d(n);
	 float *uh= sjmflloc1d(n);
	 float *ue= sjmflloc1d(n);
	 float *misfit_for   = sjmflloc1d(n);
	 float *misfit_bac   = sjmflloc1d(n);
     float *H_misfit_bac = sjmflloc1d(n);
     ...
     for(j=0;j<n;j++)
     {
     misfit_for[j] = (pow(ye[j],p)-pow(ue[j],p))*y[j]*pow(ye[j],p-2);
     misfit_bac[j] = (pow(ye[j],p)-pow(ue[j],p))*yh[j]*pow(ye[j],p-2);
     }
     hilbert (n, misfit_bac, H_misfit_bac);
     
     
     for(j=0;j<n;j++)
     {
     misfit[i][j]= misfit_for[j]-H_misfit_bac[j];
     }
     ...
     sjmfree1d(uh);
     sjmfree1d(ue);
     sjmfree1d(misfit_for);
     sjmfree1d(misfit_bac);
     sjmfree1d(H_misfit_bac);
