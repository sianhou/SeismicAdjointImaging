...
//! Two dimension constant density acoustic EI gradient
void sjaeig2d(sjssurvey *sur, sjsgeology *geo, sjswave *wav, sjsoption *opt) {
    ...
    if (rankid == 0) {
        tstart = (double) clock();
        printf("Calculating AEI gradient -                     ");
    }
    ...
    for (is = rankid; is < sur->ns; is += nrank) {
        ...
        geo->nzz2d = sjmflloc2d(sur->nx, sur->nz);
        float **recz = sjmflloc2d(sur->nr, opt->nt);
        float **csyn = sjmflloc2d(sur->nr, opt->nt);
        wav->fwz2d = sjmflloc3d(opt->nsnap, sur->nx, sur->nz);
        ...
        //! Simulation
        sjafor2d(sur, geo, wav, opt);
        memcpy(csyn[0], wav->profz[0], sur->nr * opt->nt * sizeof(float));
